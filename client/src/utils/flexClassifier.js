// Flex classification utility
//"EF_Flex", "IF_Flex", "MF_Flex", "PF_Flex", "RF_Flex", "TF_Flex", "WF_Flex"
export const classifyFlexValue01 = (value) => {
  if (value < 5000) return "No Bend";
  if (value < 3200) return "Almost No Bend";
  if (value < 1400) return "Almost Full Bend";
  return "Full Bend";
};
export const classifyFlexValue02 = (value) => {
  if (value > 6000) return "No Bend";
  if (value > 3200) return  "Almost No Bend";
  if (value > 1400) return "Almost Full Bend";
  return "Full Bend";
};
export const classifyFlexValue03 = (value) => {
  if (value > 8000) return "No Bend";
  if (value > 6000) return  "Almost No Bend";
  if (value > 4000) return "Almost Full Bend";
  return "Full Bend";
};
export const classifyFlexValue04 = (value) => {
  if (value > 5500) return "No Bend";
  if (value > 2000) return  "Almost No Bend";
  if (value > 600) return "Almost Full Bend";
  return "Full Bend";
};
export const classifyFlexValue05 = (value) => {
  if (value > 4500) return "No Bend";
  if (value > 3000) return  "Almost No Bend";
  if (value > 2000) return "Almost Full Bend";
  return "Full Bend";
};
export const classifyFlexValue06 = (value) => {
  if (value > 1000) return "No Bend";
  if (value < 500) return  "Almost No Bend";
  if (value < 0) return "Almost Full Bend";
  return "Full Bend";
};
export const classifyFlexValue07 = (value) => {
  if (value < 5000) return "No Bend";
  if (value < 3200) return  "Almost No Bend";
  if (value < 1400) return "Almost Full Bend";
  return "Full Bend";
};
